...
func TestStdColumnBlock(t *testing.T) {
	...
	for i := 0; i < seg_cnt; i++ {
		...
		blk_0 := NewStdColumnBlock(seg, blk_0_id, PERSISTENT_BLK)
		...
		blk_1 := NewStdColumnBlock(seg, blk_1_id, PERSISTENT_BLK)
...
func TestStrColumnBlock(t *testing.T) {
	...
	for i := 0; i < seg_cnt; i++ {
		...
		assert.Nil(t, seg.GetBlockRoot())
		blk_0_id := seg_id.NextBlock()
		blk_0 := NewStrColumnBlock(seg, blk_0_id, PERSISTENT_BLK)
